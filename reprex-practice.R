library(tidyverse)
library(palmerpenguins)
library(reprex)
#wrote a bunch of code

#finally at the end i want to plot results
ggplot(data = penguins, aes(x= flipper_length_mm, y= body_mass_g)) +
  geom_point()
ggplot(data = penguins, x= flipper_length_mm, y= body_mass_g) +
  geom_point()

#create data to include in our reprex
#subset penguins to just the. data i need to reproduce the error
penguins_subset <- penguins %>%
  select(flipper_length_mm, body_mass_g) %>%
  slice_head(n=5)

#creating a version of the data to include dirrectlty in reprex
datapasta::df_paste(penguins_subset) #creates data below

# CREATING REPREX
#What were actually going to share

library(tidyverse)

penguins<- data.frame(
  flipper_length_mm = c(181L, 186L, 195L, NA, 193L),
        body_mass_g = c(3750L, 3800L, 3250L, NA, 3450L)
)

ggplot(data = penguins, x= flipper_length_mm, y= body_mass_g) +
  geom_point()


#reprex: made by copying above then doing reprex() in console
``` r
library(tidyverse)

penguins<- data.frame(
  flipper_length_mm = c(181L, 186L, 195L, NA, 193L),
  body_mass_g = c(3750L, 3800L, 3250L, NA, 3450L)
)

ggplot(data = penguins, x= flipper_length_mm, y= body_mass_g) +
  geom_point()
#> Error in `geom_point()`:
#> ! Problem while setting up geom.
#> ℹ Error occurred in the 1st layer.
#> Caused by error in `compute_geom_1()`:
#> ! `geom_point()` requires the following missing aesthetics: x and y.
```

<sup>Created on 2025-08-21 with [reprex v2.1.1](https://reprex.tidyverse.org)</sup>

mtcars <- mtcars
  ggplot(data = mtcars, aes(x= cyl, y= hp)) +
  geom_histogram()

  mtcars_subset <- mtcars %>%
    select(cyl, hp) %>%
    slice_head(n=5)
  datapasta::df_paste(mtcars_subset) #creates data below

  
  
  
  
  library(tidyverse)
  
  mtcars_subset <- data.frame(
                                row.names = c("Mazda RX4",
                                              "Mazda RX4 Wag","Datsun 710",
                                              "Hornet 4 Drive","Hornet Sportabout"),
         cyl = c(6, 6, 4, 6, 8),
          hp = c(110, 110, 93, 110, 175)
                             )
  ggplot(data = mtcars, aes(x= cyl, y= hp)) +
    geom_histogram()

 
    
    